...
@inline function area(points::Union{<:AbstractVector{T}, NTuple{N, T}}) where {T <: Vec, N}
    L = length(points)
...
@inline function area(points::AbstractVector{T}, indices::VecOrTuple) where {T <: Vec}
...
@inline function area(points::AbstractVector{T}, indices::VecOrTuple, x_period::Number, y_period::Number) where {T <: Vec}
    @inbounds p1 = points[indices[1]]
...
"""
@inline function integrate(ρ::F, points::AbstractVector{T}) where {F <: Function, T <: Vec}
...
"""
@inline function centroid(points::AbstractVector{T}) where {T <: Vec}
...
"""
@inline function mass_centroid(ρ::F, points::AbstractVector{T}) where {F <: Function, T <: Vec}
...
"""
@inline function centroid(points::AbstractVector{T}, indices::VecOrTuple) where {T <: Vec}
...
The result is an approximation due to the assumption that `ρ` is linear inside each triangle that forms the polygon.
"""
@inline mass_centroid(ρ::F, points::AbstractVector{T}, indices::VecOrTuple) where {F <: Function, T <: Vec} = mass_centroid(ρ, @view(points[indices]))

@inline function centroid(points::AbstractVector{T}, indices::VecOrTuple, x_period::Number, y_period::Number) where {T <: Vec}
...
"""
function in_polygon(p::Vec, points::AbstractVector{T}, indices::VecOrTuple) where {T <: Vec}
    @inbounds p1 = points[indices[1]]
    @inbounds p2 = points[indices[2]]
...
"""
function in_polygon(p::Vec, points::AbstractVector{T}, indices::VecOrTuple, x_period::Number, y_period::Number) where {T <: Vec}
    @inbounds p1 = points[indices[1]]
    @inbounds p2 = closest(p1, points[indices[2]], x_period, y_period)
